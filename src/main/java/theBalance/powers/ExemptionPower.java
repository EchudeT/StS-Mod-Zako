...
// 豁免 - Exemption
// 本回合下一张牌的负面效果不再生效
public class ExemptionPower extends AbstractPower implements CloneablePowerInterface {
    public static final String POWER_ID = BalanceMod.makeID("ExemptionPower");
    ...
    public ExemptionPower(final AbstractCreature owner) {
        name = NAME;
        ID = POWER_ID;
        this.owner = owner;
        this.amount = 1;
    ...
    public void onUseCard(AbstractCard card, UseCardAction action) {
        if (!triggered && this.amount > 0) {
            // 标记已触发，下一张牌的负面效果将被豁免
            // 实际实现需要在卡牌使用时检查此Power
            triggered = true;
            this.amount--;
            if (this.amount <= 0) {
    ...
    public AbstractPower makeCopy() {
        return new ExemptionPower(owner);
    }
}
